...
external_dcl		: function_def				{ semantic(4); }
			...
			| TIDENT TSEMICOLON			{ semantic(600); };
			| TIDENT error				{ yyerror("ERROR: Missing semicolon for external declaration"); };
function_def		: function_header compound_st		{ strcpy(function_name, ""); semantic(6); }
			...
			| function_header error			{ yyerror("ERROR: Missing semicolon for function definition"); };
...
short_formal_param		: TLPAREN short_opt_formal_param TRPAREN	{ semantic(602); };
short_opt_formal_param	: type_specifier				{ add_param_list("(null}", tmp_type); semantic(603); }
			| type_specifier TCOMMA short_opt_formal_param	{ add_param_list("(null}", tmp_type); semantic(604); };
...
formal_param		: TLPAREN opt_formal_param TRPAREN	{ semantic(17); }
			| TLPAREN opt_formal_param		{ yyerror("ERROR: Not closed parenthesis for formal parameter"); };
...
formal_param_list       	: param_dcl				{ add_param_list(tmp_name, tmp_type); semantic(20); }
			...
			| formal_param_list param_dcl		{ yyerror("ERROR: Missing comma for formal param list"); };
param_dcl		: dcl_spec declarator			{ semantic(22); };
compound_st		: TLBRACE opt_dcl_list opt_stat_list TRBRACE	{ semantic(23); }
			| TLBRACE opt_dcl_list opt_stat_list error	{ yyerror("ERROR: Not closed brace for compound statement"); };
...
declaration		: dcl_spec init_dcl_list TSEMICOLON		{ semantic(28); }
			| dcl_spec init_dcl_list error			{ yyerror("ERROR: Missing semicolon for declaration"); };
init_dcl_list		: init_declarator				{ semantic(29); }
			| init_dcl_list TCOMMA init_declarator		{ semantic(30); }
			| init_dcl_list init_declarator			{ yyerror("ERROR: Missing comma for init declaration list"); };
...
declarator			: TIDENT					{ strcpy(tmp_name, prev_yytext); semantic(33); }
			...
			| TIDENT TLBRACKET opt_number error		{ yyerror("ERROR: Not closed bracket for declarator"); };
...
expression_st	    	: opt_expression TSEMICOLON		{ semantic(46); }
			| expression error				{ yyerror("ERROR: Missing semicolon for expression statement"); };
...
while_st		    	: TWHILE TLPAREN expression TRPAREN statement			{ semantic(51); }
			| TWHILE TLPAREN expression error statement	{ yyerror("ERROR: Not closed parenthesis for while statement"); }
			| TWHILE TLPAREN TRPAREN statement	{ yyerror("ERROR: Condition doesn't exists for while statment"); };
return_st			: TRETURN opt_expression TSEMICOLON	{ semantic(52); }
			| TRETURN opt_expression error		{ yyerror("ERROR: Missing semicolon for return statement"); };
...
postfix_exp 		: primary_exp 				{ semantic(85); }
			...
			| postfix_exp TLBRACKET expression error		{ yyerror("ERROR: Not closed bracket for postfix expression"); }
			| postfix_exp TLPAREN opt_actual_param TRPAREN	{ semantic(87); }
			| postfix_exp TLPAREN opt_actual_param error		{ yyerror("ERROR: Not closed parenthesis for postfix expression"); };
...
primary_exp 		: left_hand				{ semantic(510); }
			...
			| TLPAREN expression error			{ yyerror("ERROR: Not closed parenthesis for primary parameter"); };
%%
void semantic(int n)
{
